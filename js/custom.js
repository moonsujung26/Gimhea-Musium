$(function () {
    $('#mainFull').fullpage({
        anchors: ['mainVisaul', 'mainExhibition', 'mainEvent', 'mainIntroduction', 'mainResevation'],
        scrollingSpeed: 1000,
        afterLoad: function (name, i) {
            $('.aside li')
                .eq(i - 1)
                .addClass('on')
                .siblings()
...
$(function mainResevation() {
    const mainResevation = new Swiper('.mainReSlide', {
        effect: 'fade',
        speed: 1000,
        loop: true,
        autoplay: {
...
$(function Footer() {
    $('.footer .fb .link .first_link>a').on('click', function (e) {
        e.preventDefault();
